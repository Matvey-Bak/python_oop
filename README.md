...
## Как это работает 
### Конструктор
После объявления нашего класса первое что нам необходимо сделать это сделать конструктор для него.
В этом конструкторе будут лежать атрибуты класса которые мы будем использовать далешь в программе.
Также в конструктре мы проводим валидацию даных это делается для того чтобы при получении некорректной
записи выпадала ошибка и программа завершала свое выполнение.

### Атрибуты класса
Также в данном блоке кода мы объявляем атрибуты класса для подсчта обещего числа заказов и общей суммы заказов
...
```
## Как это работает 
Данные методы нужны чтобы дать контролируемый доступ к защищенным переменным также декоратор @property 
помогает нам обращаться к переменным как к атрибутам хоть у нас и написано что это функции вот что будет при вызове данных методов

![alt text](images/lab01/Getter&Setter.png)
...
```python
    ...
    def pay_order(self, pin_check):
        ...
        if not self._verify_pin(pin_check):
...
## Как это работает 
В даных методах мы используем атрибуты из конструктора и методы проверки состояния после чего 
если все проверки пройдены меняем состояние. Вот как это будет работать 
...
```
## Как это работает 
Данные магические методы нужны для красивого вывода в консоль для пользователя,
для вывода информации атрибутов объекта разработчику и последний метод нужен для проверки равенства id объектов.
Вот что выведет код:
...
## 5. Вывод 
В ходе лабораторной работы был разработан класс Order с следующими характеристиками:
### Инкапсуляция: защищенные атрибуты (_email, _status, _balance, _pin) с контролируемым доступом через геттеры
## Валидация данных:
* Отдельные функции для проверки каждого атрибута
* Проверка типов данных
* Проверка диапазонов значений
* Информативные сообщения об ошибках

## Бизнес-логика:
* Оплата заказа с проверкой пин-кода и баланса
...
### Магические методы: реализованы __str__, __repr__, __eq__ для удобной работы с объектами
